...
All seeded users (seed.ts) share the same hardcoded password (check the seed script's password-hashing call for the literal value — do not hardcode it here); emails follow the pattern `<role>@cenografica.com.br` (admin, rh, diretoria, visualizador, avaliador, avaliador2, avaliador.<area>).

**Why:** seed.ts hashes one shared password for every user, so there's no per-user credential to look up — that single password unlocks any seeded account for manual/API testing.

**How to apply:** `POST /api/auth/login` with `{ email, password: <seed password from seed.ts> }` to get a JWT for manual endpoint testing.
