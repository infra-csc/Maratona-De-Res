...
**Scope filter (event-year driven):** the sync only imports (1) events of `TARGET_YEAR` (2026), (2) participations in those events whose function is "Cenotécnica" or "Cenotécnica Local" (`ALLOWED_FUNCTIONS`, accent/case-insensitive), and (3) the collaborators referenced by those kept participations. Everything else is ignored. **Why:** the Maratona's real goal is "the year's events and who worked them in the cenotécnica role." To change scope, edit `TARGET_YEAR` / `ALLOWED_FUNCTIONS` in integration.ts.
