...
**Editing a penalty-type's point value retroactively:** since points snapshot per-row, editing the catalog value in `penalty_types` naturally only affects future launches. To let admins optionally apply a change retroactively too, `PUT /penalty-types/:id` accepts `applyScope: "future"|"cycle"` (only meaningful when `points` actually changed); `"cycle"` bulk-updates all `absences` rows of that `penaltyType` within the current cycle (`absences.cycleId`) and triggers `recomputeCycleResults`. Frontend only shows the scope picker when the edited value differs from the original.
