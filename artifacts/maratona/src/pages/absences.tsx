      ...
      <div className="p-6 md:p-10 space-y-8">
          ...
          <div className="flex items-center gap-5">
            ...
            <div>
              <h1 data-testid="text-page-title" className="text-4xl md:text-5xl italic uppercase tracking-tighter font-black leading-none">
                Controle de <span className="text-[#ccff00] bg-[#191c1e] px-3 inline-block -rotate-1">Faltas</span>
              </h1>
              <p className="text-base text-[#444933] italic mt-2">Lançamento de ausências que impactam diretamente no cálculo do bônus.</p>
            </div>
          </div>
                ...
                <DialogContent className="max-w-md rounded-none border-2 border-[#191c1e] shadow-[6px_6px_0px_0px_#191c1e]">
                  <DialogHeader>
                    <DialogTitle className="text-2xl italic uppercase font-black tracking-tight flex items-center gap-2 text-[#ba1a1a]">
                      <AlertTriangle size={22} /> Registrar Ausência
                    </DialogTitle>
                  </DialogHeader>
                  <form
                  ...
                  >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <Label className="font-bold italic uppercase text-xs tracking-wider text-[#444933]">Data da Falta <span className="text-[#ba1a1a]">*</span></Label>
                      ...
                      <div className="space-y-1.5">
                        <Label className="font-bold italic uppercase text-xs tracking-wider text-[#444933]">Dias (Qtd) <span className="text-[#ba1a1a]">*</span></Label>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        <Label className="font-bold italic uppercase text-xs tracking-wider text-[#444933]">Ano Contábil</Label>
                        <Input type="number" {...register("year", { valueAsNumber: true })} className="h-11 rounded-none border-2 border-[#191c1e]" />
                      </div>
                      <div className="space-y-1.5">
                        <Label className="font-bold italic uppercase text-xs tracking-wider text-[#444933]">Trimestre Contábil</Label>
                        <Select defaultValue={String(currentQuarter)} onValueChange={v => setValue("quarter", Number(v))}>
                          <SelectTrigger data-testid="select-absence-quarter" className="h-11 rounded-none border-2 border-[#191c1e] focus:ring-0">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                            <SelectItem value="1">T1</SelectItem>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input data-testid="input-absence-reason" {...register("reason")} placeholder="Atestado não entregue, falta sem justificativa..." className="h-11 rounded-none border-2 border-[#191c1e]" />
                    </div>
        ...
        <div className="flex flex-col md:flex-row gap-4 items-stretch md:items-center">
          ...
          <div className={`bg-[#ff5722] text-white px-5 py-3 border-2 border-[#191c1e] font-bold text-xs italic uppercase tracking-wider flex items-center gap-2 shrink-0 ${HARD_SHADOW}`}>
            <AlertTriangle size={16} /> Total no período: <span className="text-base not-italic">{totalAbsences}</span> faltas
          </div>
        </div>
        ...
        {isLoading ? (
          ...
          <div className={`bg-white border-2 border-[#191c1e] overflow-hidden ${HARD_SHADOW}`}>
            <div className="bg-[#191c1e] text-[#ccff00] px-6 py-3 flex items-center gap-2 italic">
              <UserMinus size={18} />
              <span className="font-black uppercase tracking-tight">Registros de Ausência</span>
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b-2 border-[#191c1e] bg-[#eceef0]">
                    ...
                    <th className="px-6 py-4 text-xs font-bold uppercase italic text-[#444933]">Data Registrada</th>
                    <th className="px-6 py-4 text-xs font-bold uppercase italic text-[#444933] text-center">Quantidade</th>
                ...
                <tbody className="divide-y-2 divide-[#eceef0]">
                  ...
                  {filteredAbsences.length === 0 && (
                    <tr><td colSpan={canEdit ? 5 : 4} className="text-center py-16 italic uppercase font-bold text-[#747a60]">Nenhuma falta registrada no período.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
